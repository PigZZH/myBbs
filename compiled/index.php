                                                    ...
                                                    foreach ($data AS $k => $v) { ?>
                                                        <?php if ($value['bid'] == $v['parentid']) { ?>
                                                            <tr>
                                                                ...
                                                                <td class="fl_i">
                                    <span
                                        class="xi2"><?php if (isset($deal[$v['cid']])) { ?><?php echo $deal[$v['cid']]; ?><?php } else { ?>0
                                        <?php } ?></span><span class="xg1"> /<?php if (isset($rela[$v['cid']])) { ?><?php echo $rela[$v['cid']]; ?>
                                                                        <?php } else { ?>0<?php } ?></span>
                                                                </td>
                                                                <td class="fl_by">
                                                                    <div>
                                                                        <a href="detail.php?id=<?php echo select_returnone($conn, 'bbs_details', 'id', 'tid ="' . $v['cid'] . '"', 'addtime
                                        desc'); ?>"
                                                                           class="xi2"><?php echo select_returnone($conn, 'bbs_details', 'title', 'tid
                                        ="' . $v['cid'] . '"', 'addtime desc'); ?></a> <cite><span
                                                                                title="2016-08-26"><?php echo Lasttime(select_returnone($conn, 'bbs_details', 'addtime', 'tid ="' . $v['cid'] . '"', 'addtime desc')); ?></span>
                                                                            <?php echo select_returnone($conn, 'bbs_details', 'authorid', 'tid
                                        ="' . $v['cid'] . '"', 'addtime desc'); ?></cite></div>
                                                                </td>
                                                            </tr>
                                                        <?php } ?>
                                                    <?php }
